...
class TotpToSamlResponseTest {
    ...
    @BeforeEach
    void setupTest() {
        TotpVerifier totpVerifier = Mockito.mock(TotpVerifier.class);
        AccountsService accountsService = Mockito.mock(AccountsService.class);

        Mockito.when(totpVerifier.verifyAccountTokenAsync(Mockito.any()))
        ...
        exchange = new TotpToSamlResponse(totpVerifier, accountsService, configuration);
    }
