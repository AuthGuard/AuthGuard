...
class BasicToSamlResponseTest {
    ...
    @BeforeEach
    void setupTest() {
        BasicAuthProvider basicAuthProvider = Mockito.mock(BasicAuthProvider.class);

        Mockito.when(basicAuthProvider.authenticateAndGetAccountSession(Mockito.any()))
                .thenReturn(Uni.createFrom().item(AccountSessionBO.builder()
                        .session(SessionBO.builder()
                                .sessionToken("token")
                                .domain("domain")
                                .build())
                        .account(AccountBO.builder()
                                .id(1)
                                .build())
                        .build()));
        ...
        exchange = new BasicToSamlResponse(basicAuthProvider, configuration);
    }
